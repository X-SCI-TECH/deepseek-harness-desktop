 ...
 *   2. 槽位 A：`conversation.chat.turnTail`（chain，priority -1）注册变更卡片 ——
 *      每轮结束处显示「已编辑 N 个文件 / +N -M / 文件清单 / 撤销 / 审核」，
 *      并以同样的优先级替换官方 “Files changed” 行；
 *   3. 槽位 B：`conversation.input.dock`（list）注册运行中提示条 ——
 *      输入框上方实时显示「N 个文件已更改 +N -M」，turn 结束后消失；
 *   4. 样式：各组件 `useMountStyle` 自挂自卸，无需全局样式 effect。
 *
