...
| 单文件标题 / 清单行 | 点击调用框架的 `openFile` **打开该文件**（临时逻辑） |
| `＋` 图标块（文件） | **占位**（无点击处理，`data-placeholder` 标注） |
| 「查看更改 ↗」（单文件 hover） | **占位**（无点击处理）；撤销前后都保留这个 hover 效果 |
| `审核` | **暂时整体隐藏**（代码内 `TODO(review-action)` 保留按钮实现） |

- 单文件：标题即文件名、不渲染清单；hover 时副行的计数换成「查看更改 ↗」。
- 多文件：标题是文件数、副行是总计（绿 `+N` / 红 `-M`）；清单最多三行，其余折叠；行 hover 高亮；本轮删除的文件整行弱化。
- **已撤销的 turn**：只剩「已撤销」徽标与文件名/清单行，不再有撤销按钮与审核（hover 的「查看更改」保留）。
- 该轮没有任何文件变化 → 不出现卡片。
- 撤销成功后同一 turn 不能重复撤销。
